...
Read and follow this file only when invoked with `mode:pipeline` (`sf-lfg` or another automated runner). It overrides visibility prompts, free-port selection, and dev-server startup. It does not change browser-driver selection. In pipeline mode you run unattended — never block on a question.
...
Before running the block, resolve `PREVIEW_KIND` from the affected surfaces: `site` for Experience Cloud routes and `app` for Lightning App Builder/LWC/Aura surfaces. Prefer a repository-provided `npm run dev` script when present. Otherwise use Salesforce Local Dev only when `sf lightning dev <kind> --help` confirms that preview command is installed and the project supplies any required site/app configuration; never guess missing org, site, or app identifiers.
...
PREVIEW_KIND=app   # use site for Experience Cloud routes
...
if [ -f "package.json" ] && node -e 'const p=require("./package.json"); process.exit(p.scripts?.dev ? 0 : 1)' 2>/dev/null; then
  PORT=${PORT} npm run dev > /tmp/dev-server-${PORT}.log 2>&1 &
elif [ -f "sfdx-project.json" ] && sf lightning dev "${PREVIEW_KIND}" --help >/dev/null 2>&1; then
  sf lightning dev "${PREVIEW_KIND}" --port "${PORT}" > /tmp/dev-server-${PORT}.log 2>&1 &
else
  echo "No configured local preview command found. Configure npm run dev or Salesforce Local Dev for this project."
  exit 1
fi
