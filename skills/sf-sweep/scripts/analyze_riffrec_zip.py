...
video frames when available, and Salesforce Compound Engineering-friendly
markdown artifacts.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted evidence/kickoff artifacts. Defaults to docs/brainstorms/riffrec-feedback/<soursf-stem> when available; durable sf-brainstorm outputs live in the plans artifact directory.",
    )
...
def write_analysis_md(
    ...
    lines.append("- Use repo-relative screenshot paths when moving evidence into a Salesforce Compound Engineering requirements document.")
...
def write_requirements_kickoff(
    ...
    source_materials = markdown_link(str(output_path.parent / "soursf-materials.md"), output_path.parent, repo_root)
    ...
    lines = [
        ...
        "## Key Flows",
        "",
        "- F1. Evidensf-backed feedback triage",
    ...
    lines.extend(
        [
            ...
            "- `sf-brainstorm` can confirm requirements from linked source evidence before any planning begins.",
            "",
            "---",
            "",
            "## Scope Boundaries",
            ...
            "- Brainstorm before plan: Use `sf-brainstorm` to refine product behavior when the recording reveals ambiguity.",
            "",
            "---",
            "",
            ...
            "## Next Steps",
            "",
            "-> Resume `/sf-brainstorm` to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    source_materials_md = output_dir / "soursf-materials.md"
    ...
    print(f"Brainstorm handoff: /sf-brainstorm {display_path(kickoff_md, repo_root)}")
