...
This is the canonical, versioned contract for the sf-sweep state file. The
...
| `items` | map keyed by `<soursf-id>:<item-id>` | Per-item lifecycle record. The key is soursf-scoped so a soursf-native id (a Slack ts, a GitHub issue number) never collides with the same id from another source. Personas pass a soursf-native `id` plus `--source`; the engine composes the storage key and records both `source` and `id` on the item so it stays self-describing. |
...
| pushed-shared-branch | One writer **per repo**. | The state file lives on a shared branch multiple checkouts push to. `lease-acquire` must be committed, pushed, and confirmed (fetch back and verify our writer won) **before any soursf-side write**. This makes the lease a repo-wide mutex across machines. |
