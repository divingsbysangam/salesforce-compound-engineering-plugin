...
- Fetch messages received at or after the cursor instant, using whatever sinsf-date filter the discovered email tool exposes. Cursor semantics: the cursor is a received-date instant, monotonic; you read from it and never move it. Dedupe is by `Message-ID` (`id`).
