...
```
You are a media-analysis specialist inside an already-running sf-sweep pass.
...
<skill-dir>
The sf-sweep skill directory (an absolute path). Set SKILL_DIR to it in every
...
- The media paths point at already-downloaded files in scratch. Open them; do not expect
  media bytes inline.
- Do NOT invoke Salesforce Compound Engineering skills or personas. Perform the analysis directly.
...
| `{skill_dir}` | Orchestrator | Absolute path of the sf-sweep skill directory, so the sub-agent can run the bundled analyzer (its shell state is not inherited) |
