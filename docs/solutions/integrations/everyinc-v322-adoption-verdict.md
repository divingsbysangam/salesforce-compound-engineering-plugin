...
**After (right):** Shallow-fetch tag `compound-engineering-v3.22.4` (and note `main` if the 8KB sweep is still untagged). Produce a compare-only matrix like the 3.21 audit. Spike `sf-babysit-pr` by splitting the 75,843-byte always-loaded `SKILL.md` until it is under the host cap. Cherry-pick equivalent bugfixes into existing `sf-*` skills. Leave Apex/LWC/Flow/Agentforce skills untouched.
